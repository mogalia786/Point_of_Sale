...
If c1.Value = 1 Then
    With r
    .Open "select * from creditorspayment order by paymentdate", c, adOpenDynamic, adLockOptimistic
    End With
Else
    With r
    .Open "select * from creditorspayment where paymentdate>='" & Year(dFROM.Value) & "-" & Month(dFROM.Value) & "-" & Day(dFROM.Value) & " 00:00:00.000' and paymentdate<='" & Year(dTO.Value) & "-" & Month(dTO.Value) & "-" & Day(dTO.Value) & " 00:00:00.000' order by paymentdate", c, adOpenDynamic, adLockOptimistic
    End With
End If
